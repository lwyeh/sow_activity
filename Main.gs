...
var ROLE_LIST    = ['複式團長','會長','蟻育副會長','蜂育副會長','鹿育副會長','鷹育副會長','手作組組長','財務組組長','資訊組組長','文書組組長','值星官','財務','安心營營長','安心營','副團長','團長','季總召導引員','活動總召','助理導引員','導引員','小小蟻','小蟻','小蜂','小鹿','小鷹','蟻蜂育','鹿育','鷹育','離團','無'];
var SQUAD_LIST   = ['花叢小隊','天空小隊','草原小隊','大地小隊','草原鹿','森林鹿','高地鹿','湖泊鹿','泥壺蜂','虎頭蜂','長腳蜂','細腰蜂','小黑蟻','小黃蟻','小綠蟻','小紅蟻','鹿育','鷹育','小鷹','無'];
var GENDER_LIST  = ['男','女'];
...
function setupSheets() {
  ...
  var sheetsConfig = {
    ...
    '成員資料': ['成員ID', '家庭ID', '家庭名稱', '姓名', '自然名', '性別', '角色', '電話', 'Email', '團別', '年級', '職位', '隊名', '建立時間', '會員編號', '出生年月日', '入團日期', '個人備註'],
    '活動列表': ['活動ID', '活動名稱', '開始日期', '結束日期', '截止報名日期', '建立時間', '開放報名日期', '納入統計'],
    '報名紀錄': ['報名ID', '活動ID', '活動名稱', '成員ID', '成員姓名', '家庭ID', '家庭名稱', '出席狀態', '備註', '報名時間', '最後更新'],
    '收費活動': ['活動ID', '活動名稱', '說明', '類型', '建立時間', '繳費方式', '開放日期', '截止日期'],
  ...
  for (var sheetName in sheetsConfig) {
    ...
    switch (sheetName) {
      case '成員資料':
        // H 欄是電話，強制設為純文字，避免 09 開頭的 0 被消失
